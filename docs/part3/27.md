...
这一章我们继续介绍计算着色器的知识点，对于双调排序部分，读者可以选择性阅读。
...
在一个线程组内，允许设置一片组共享内存区域，使得当前线程组内的所有线程都可以访问当前的组共享内存。一旦设置，那么每个线程组都会各自配备一份组共享内存。组共享内存的访问速度非常快，接近于GPU访问寄存器的水平。
...
对于每个线程组来说，它所允许分配的总空间最大为32kb(即8192个标量，或2048个向量)。内部线程通常应该使用`SV_GroupThreadID`来写入组共享内存，这样以保证每个线程不会出现重复写入操作，而读取组共享内存一般是线程安全的。

分配太多的组共享内存会导致性能问题。假如一个多处理器支持32kb的组共享内存，然后你的计算着色器需要20kb的组共享内存，这意味着一个多处理器只适合处理一个线程组，因为剩余的组共享内存不足以给新的线程组运行，这也会限制GPU的并行运算，当该线程组因为某些原因需要等待，会导致当前的多处理器处于闲置状态。因此保证一个多处理器至少能够处理两个或以上的线程组(比如每个线程组分配16kb以下的组共享内存)，以尽可能减少该多处理器的闲置时间。
...
因为多个线程同时运行，同一时间各个线程当前执行的指令有所偏差，有的线程可能已经完成了组共享内存的赋值操作，有的线程可能还在进行纹理采样操作。如果当前线程正在读取相邻的组共享内存片段，结果将是未定义的。为了解决这个问题，我们必须在读取组共享内存之前让当前线程等待线程组内其它的所有线程完成写入操作。这里我们可以使用`GroupMemoryBarrierWithGroupSync`函数：
...
现在我们先从核心排序算法讲起，由于受到线程组的线程数目、组共享内存大小限制，这里定义一个线程组包含512个线程，即一个线程组最大允许排序的元素数目为512。组共享内存在这是用于临时缓存中间排序的结果。
...
最后就是要确定当前线程对应的组共享内存元素需要得到较小值，还是较大值了。这里又以一个双调序列$(2, 5, 7, 4)$为例，待比较的两个元素为5和4，当前趟排序会将它变为单调递增序列，即所处的序列为递增序列，当前线程对应的元素为5，`shared_data[smallerIndex] <= shared_data[largerIndex]`的比较结果为`>`，那么它将拿到(较小值)较大索引的值。经过第一趟交换后将变成$(2, 4, 7, 5)$，第二趟交换就不讨论了。
...
最后是`std::sort`和双调排序(使用NVIDIA RTX 3080Ti)的比较结果，这回给GPU排序使用了GPU Timer，测试相对更加准确一些：

| **元素数目** | **CPU排序（s）** | **GPU排序（s）** | **GPU排序+映射回内存总时（s）** |
| ------------ | ---------------- | ---------------- | ------------------------------- |
| 512          | 0.000017         | 0.000043         | 0.002 到 0.017                  |
| 1024         | 0.000037         | 0.000055         | 0.002 到 0.017                  |
| 2048         | 0.000068         | 0.000068         | 0.002 到 0.032                  |
| 4096         | 0.000151         | 0.000084         | 0.002 到 0.032                  |
| 8192         | 0.000328         | 0.000093         | 0.003 到 0.034                  |
| 16384        | 0.000698         | 0.000109         | 0.003 到 0.034                  |
| 32768        | 0.001415         | 0.000134         | 0.003 到 0.034                  |
| 65536        | 0.002921         | 0.000173         | 0.003 到 0.034                  |
| 131072       | 0.006647         | 0.000261         | 0.003 到 0.034                  |
| 262144       | 0.013044         | 0.000416         | 0.01 到 0.034                   |


可以初步看到双调排序的排序用时比较稳定，而快速排序明显随元素数目增长而变慢。当然，如果GPU排序的数据量再更大一些的话，可以看到时间的明显增长。

**但是！如果你用GPU排序后需要取回到CPU，因为GPU->CPU的速度比较慢，并且需要等待资源结束占用才能开始取出。一般来说在GPU运算的数据能够在GPU使用会更好**
