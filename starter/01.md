...
| 章节                                                         |
| ------------------------------------------------------------ |
| [教程项目无法编译、运行的解决方法](https://www.cnblogs.com/X-Jun/p/10189305.html) |
| [HLSL语法入门](https://www.cnblogs.com/X-Jun/p/12246859.html) |
| [ComPtr智能指针](https://www.cnblogs.com/X-Jun/p/10189859.html) |
| [HR宏关于dxerr库的替代方案](https://www.cnblogs.com/X-Jun/p/10170535.html) |
...
![](../Images/1172605-20190412090511814-1568191909.png)

编程语言那一项会自动被勾选。

而如果是使用VS2017或19的话，则在安装的时候需要勾选下列选项：

![](../Images/1172605-20190412090732766-1578829189.png)

> **注意：VS2015使用的Windows SDK版本为10.0.14393.0，VS2017和VS2019可以使用当前你所安装的Windows SDK版本**

安装完成后，新建项目从Windows桌面向导开始：

![](../Images/1172605-20200718221943804-1294450901.png)

在填写好项目名称后，点击创建会弹出这样一个子窗口。我们需要按下图的方式操作来创建出一个空项目的桌面应用程序，最后点击确定即可：

![](../Images/1172605-20200718222008933-36968584.png)
...
这里的每一个项目都需要包含静态库：`d3d11.lib`,`dxgi.lib`,`dxguid.lib`,`D3DCompiler.lib`和`winmm.lib`。可以在`d3dApp.h`添加下面的语句：
...
![](../Images/1172605-20181222222131075-159253660.png)

## 字符集设置为Unicode

在项目属性页中可以直接进行修改。

![](../Images/1172605-20181222222206381-1198412375.png)

## Win7系统下的额外配置

由于Win10 SDK中的某些函数在Win7是不支持的，我们还需要在属性页-配置属性-C/C++ -预处理器中，添加预处理器定义以限制API集合：`_WIN32_WINNT=0x601`

![](../Images/1172605-20181228110808689-1958314464.png)

## 文件本身的编码设置及编译时编码识别

**现在你可以尝试将本教程项目01中用到的所有头文件和源文件复制到你的项目目录，再把它们拖进你的项目中。然后你需要按下面的部分修改。**

**在项目属性页-C/C++ -命令行中添加`/utf-8`来强制指定代码页**

![](../Images/1172605-20200103113450336-1590751113.png)

一般拖过去的代码文件，即所给项目下的所有*.cpp, *.h, *.hlsl, *.hlsli使用的文件编码为**Unicode(UTF-8无签名)-代码页65001**.但从VS创建出来的文件默认编码是ANSI编码（详细的为**简体中文(GB2312)-代码页936**），需要对非UTF-8无签名编码的文件进行转码。

有一种比较麻烦的方法是，在解决方案资源管理器中选中指定文件后，点击文件-另存为-编码保存，最后找到所需编码即可。

![](../Images/1172605-20200103112707852-1235352034.png)

**当然更理想的办法是在VS安装扩展。在扩展-管理扩展，然后找到Force UTF-8(No BOM)安装即可**：

![](../Images/1172605-20200128000037757-1817278346.png)

安装该插件后，在你保存代码的时候就会强制修改文件编码为UTF-8了。

> 注意：HLSL编译器使用的是ANSI编码，使用UTF-8编码的HLSL文件如果只是注释为中文还是没有问题的，但不能在代码部分出现中文，否则编译不通过。
...
![](../Images/1172605-20200105154719681-1658458656.png)

因为默认的空项目指定的子系统有可能是未明确，也可能是控制台（用的是传统的`int main()`型函数而不是`WinMain`）。为此还需要修改项目配置。

![](../Images/1172605-20200105154859068-1738342433.png)

## 安装HLSL扩展

点击菜单栏扩展-管理扩展，搜索`HLSL Tools for Visual Studio`并安装，这样后续查看、编写HLSL文件就拥有了更加丰富的代码高亮和提示。

![](../Images/1172605-20200718222038824-2060286794.png)
...
![](../Images/1172605-20191227095430363-1348488784.png)
...
![](../Images/1172605-20190412094917001-570041129.png)
...
![](../Images/1172605-20190412121440039-1684531380.png)
...
![](../Images/1172605-20191227105049759-1197518809.png)
...
![](../Images/1172605-20181111163838124-826515353.png)
...
3. 一个显示适配器可能关联了多个输出设备(`IDXGIOutput`)，你可以使用`IDXGIAdapter::EnumOutputs`方法来枚举出特定的输出，尝试观察它们的信息。
   ![](../Images/1172605-20181120125051905-1850534891.png)
